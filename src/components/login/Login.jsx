...
export default function Login() {
	...
	useEffect(() => {
		setIsButtonEnabled(formData.email !== '' && formData.password !== '' && captchaValue !== '');
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (response?.status == HTTP_CODE.SUCCESS && `${response?.data?.status_code}` === '112') {
	...
	let continueWithGoogle = useGoogleLogin({
		onSuccess: async (response) => {
			...
			const user_type_code = res?.data?.user_type_code || res?.response?.data?.user_type_code;
			if (res && res.status == HTTP_CODE.SUCCESS && user_type_code === 1002) {
				...
				const errorMessage = `${user_type_code}` === '1004'
	...
	return (
		<div className="w-full min-h-screen flex flex-col md:flex-row">
			...
			<div className="hidden md:flex flex-col w-1/2 h-full lg:px-16 lg:py-16 md:px-10 md:py-12 bg-white">
				<div>
					<h1 className="text-purple-primary text-4xl font-bold mb-12">Why Xcellify?</h1>
					<div className="flex flex-col items-center space-y-10">
						{data.map((item, index) => (
							<LoginLeftItems key={index} img={item.img} content={item.content} />
						))}
					</div>
				</div>
				<div className="flex flex-col items-center justify-center w-full">
					<img src={Icon} alt="Xcellify Logo" onClick={()=>{navigate('/')}} className="w-[300px] cursor-pointer" />
				</div>
			</div>
			...
			<div className="w-full min-h-screen md:w-1/2 flex flex-col items-center max-md:justify-center lg:pt-16 md:pt-12 md:px-4 bg-gradient-to-b from-purple-primary to-[#6C59CA]">
				...
				<form className="w-full max-w-lg" onSubmit={(e) => e.preventDefault()}>
					...
					<div className="flex items-center rounded-xl bg-[#F5F5F5]">
						...
						<input
							...
							onChange={handleChange}
						/>
						<span onClick={togglePasswordVisibility} className="cursor-pointer pe-4">
							<img src={isPasswordVisible ? eyeOn : eyeOff} alt="Toggle Visibility" className="w-8 h-6" />
						</span>
					</div>
					<div className="text-right mt-2 mb-5 mr-5">
						<p
							onClick={handleClick}
							className="text-white text-sm cursor-pointer">Forgot password?</p>
					</div>
					<Captcha
					...
					<button
						...
						className={`w-full text-white py-3 rounded-xl mt-10 transition-all ${isButtonEnabled ? 'bg-[#6A3CB3] hover:bg-[#57359E]' : 'bg-gray-400 cursor-not-allowed'
							}`}
						type="button"
					...
					>
						{loading ? (
							<div className="flex items-center justify-center">
								<svg aria-hidden="true" className="w-6 h-6 text-gray-200 animate-spin fill-white" viewBox="0 0 100 101">
				...
				<div className="text-center mt-8">
					<p className="text-white">Don't have an account? <Link to="/signup" className="font-medium underline">Sign Up</Link></p>
				</div>
			</div>
			<dialog className="items-center justify-center w-font-dmsans rounded-lg shadow-md"
				ref={emailDialog}>
