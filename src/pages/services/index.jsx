    ...
    const verifyOtp = async (otp) => {
        try {
            const response = await apiRequest({
                ...
                method: "post",
                data: { orderId: selectedService?.orderId, sellerId: selectedService?.sellerId, otp: parseInt(otp) }
            });
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            <main className="flex-1">
                ...
                <ServiceModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onVerify={verifyOtp} />
            </main>
        </div>
    );
}
