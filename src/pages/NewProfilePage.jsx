...
export default function ProfilePage() {
  ...
  const debouncedPincode = useDebouncedValue(editProfileData.pincode || '', 500)
  ...
  const handleSaveAndUpdate = async (e)=>{
    if(checkFieldErrors() || Object.keys(formData).length === 0)
      return;
      ...
      <main className="w-full bg-gray-50">
        ...
        <div className="w-full flex flex-col items-center">
          <h2
            ...
            >{sections[activeTab].title}</h2>
          <div className="w-full sm:grid sm:grid-cols-2 sm:gap-x-10 gap-y-10 grid-cols-1 items-center">
            {(editSubSection!=2) &&
              <>
              <VendorEditForm
              fields={steps[tabsIndex[activeTab]].sections[editSubSection].fields}
