...
export default function ResetPassword() {
    ...
    let handleSubmit = async ()=>{
        ...
        console.log(status)
        const info = (status)?
        (status === HTTP_CODE.SUCCESS)? {message:"Password Updated Successfully", color:"text-green-700"}
        : (status === HTTP_CODE.TOKEN_INVALID) ? {message:"The reset link has expired", color:"text-blue-500"}
        :{message:"Internal server error, Please try again", color:"text-red-700"}
        : {message:"An error occured, Please try again", color:"text-red-700"}
        setInfoMessage(info)
    }
  ...
  return (
    <div className='flex items-center justify-center h-screen font-dmsans'>
        <div className="flex flex-col p-5 bg-white rounded-lg shadow-md dark:border max-w-xl w-full sm:p-20">
            ...
            <input className="bg-[#E5E5E5] text-l px-2 font-normal outline-0 py-3 rounded-lg mt-4 sm:px-7"
             type='password'
            ...
            {infoMessage.message != "" &&
                <p className={`text-center font-medium text-l mt-4 ${infoMessage.color}`}>{infoMessage.message}</p>
            }
            <p className='text-center font-medium text-l mt-4'>Back to login?
                <Link to="/login" className='text-purple-primary'>Login</Link>
            </p>
        </div>
